...
dat %>%
  ...
  group_by(westost, Variable, Wert, Variable2, Wert2) %>%
  summarise(n = sum(Faktor_w123456), N=n()) %>%
  mutate(p = round(n/sum(n)*100), N=sum(N)) %>%
  pivot_wider(names_from=Wert2, values_from=p, id_cols=c(Wert, Variable2, N, westost)) %>%
  ...
  dat %>%
    ...
    mutate(
      Wert2 = case_when(
        ...
        Wert2 %in% 4:5 ~ "großes oder sehr großes Vertrauen")) %>%
    group_by(westost, Variable, Wert, Variable2, Wert2) %>%
    summarise(n = sum(Faktor_w123456), N=n()) %>%
    mutate(p = round(n/sum(n)*100), N=sum(N)) %>%
    pivot_wider(names_from=Wert2, values_from=p, id_cols=c(Wert, Variable2, N, westost)) %>%
    select(westost, Wert, Variable2, N, `großes oder sehr großes Vertrauen`, `mittelmäßiges Vertrauen`, `wenig oder kein Vertrauen`) %>%
    mutate(
      Variable2 = sapply(Variable2, function(x) attributes(dat[[x]])$label),
