...
SPD_Personen <- 
  ...
  mutate(spd = 
         ...
         ostwest = if_else(bundesland %in% levels(bundesland)[1:11], "West inkl. Berlin", "Ost"), 
         zufriedenheit_regierung = if_else(zufriedenheit_regierung %in% levels(zufriedenheit_regierung)[1:2], 
                                           "Im Schnitt sehr zufrieden/zufrieden", "Im Schnitt weniger zufrieden / gar nicht zufrieden"),
         migration = case_when(migration %in% levels(migration)[1:3]~"Ja", migration %in% levels(migration)[4]~"Nein")) %>% 
  filter(!is.na(spd)) %>%
  pivot_longer(cols=c(gewerkschaft:erwerbsstatus, zufriedenheit_regierung:alter, ostwest), 
...
dat %>%
  ...
  filter(!is.na(spd) & !is.na(ostwest)) %>%
  pivot_longer(cols=c(bildung, nettoeinkommen), 
               names_to="Variable", values_to="Wert") %>%
  filter(!is.na(Wert)) %>%
  group_by(ostwest, spd, Variable, Wert) %>%
  summarise(n = sum(Faktor_w123456), N = n()) %>%
  mutate(p = round(n/sum(n)*100, digits=1), N = sum(N)) %>%
  ungroup() %>% 
  pivot_wider(names_from = spd, values_from = p, 
              id_cols=c(ostwest, Wert)) %>%
  filter(!is.na(Wert))
