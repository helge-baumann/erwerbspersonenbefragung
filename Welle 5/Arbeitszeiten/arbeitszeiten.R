...
Ergebnis <- dat %>%
  ...
  summarise_at(
    vars(starts_with(az)),
    funs(wtd.mean(., w = gewicht, na.rm = T))
  ) %>%
  filter(!is.na(Wert))
...
names(Ergebnis)[3:ncol(Ergebnis)] <-
  paste(
    months(seq(as.Date("2020/02/01"), by = "month", length.out = ncol(Ergebnis) - 2)),
    format(seq(as.Date("2020/02/01"), by = "month", length.out = ncol(Ergebnis) - 2),
      format = "%Y"
    )
  )
...
writeDataTable(wb, "Arbeitszeiten", Ergebnis)
