...
abgleich <- openxlsx::read.xlsx(
  paste0(
  "Input/", 
  "2021-07-18_Erwerbspersonenbefragung_Abgleich-Welle12345.xlsx"
  ), sheet=1, 
  startRow=2
)
...
attributes(dat_long$Faktor_w12345)$label <- "Wellenspezifischer Gewichtungsfaktor"
attributes(dat_long$lfdn_W2_w2345)$label <- "Wellenspezifische ID"
attributes(dat_long$Interviewtag_w2345)$label <- "Datum des Interviews"

write_dta(dat_long, "./Daten_Export/Erwerbspersonenbefragung_Wellen1-5_long_v1-1.dta")
write_sav(dat_long, "./Daten_Export/Erwerbspersonenbefragung_Wellen1-5_long_v1-1.sav")
