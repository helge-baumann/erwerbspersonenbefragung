...
abgleich <- openxlsx::read.xlsx(
  paste0(
    "Input/", 
    "2022-08-02_Erwerbspersonenbefragung_Abgleich-Welle12345678.xlsx"
  ), sheet=1, 
  startRow=2
)

names(abgleich) <- c("n1", "l1", "n2", "l2", "n3", "l3", "n4", "l4", "n5", "l5",
                     "n6", "l6", "n7", "l7", "n8", "l8","erh", "iden", "anm")
...
#dat_long 
dat_long_full <- dat_full %>% select(-ID__8) %>%
  pivot_longer(
    cols = -ID, 
...
attributes(dat_long_full$Faktor_w12345678)$label <- "Wellenspezifischer Gewichtungsfaktor (ohne Aufstockungsstichprobe)"
attributes(dat_long_full$lfdn_W2_w2345678)$label <- "Wellenspezifische ID"
attributes(dat_long_full$Interviewtag_w2345678)$label <- "Datum des Interviews"
...
attributes(dat_long_full$W5_S12o_w57)$label <- "Sonntagsfrage: andere Partei, und zwar: (offen)"
attributes(dat_long_full$ziehung_w5)$label <- "Nachziehung Welle 5"

attributes(dat_long_full$ziehung_w5)$labels <- 
  setNames(c(0,1), c("Ursprüngliche Stichprobe (Welle 1)", "Nachziehung (Welle 5)")) 

attributes(dat_long_full$Faktor_voll_w578)$label <- "Gewichtungsfaktor inklusive Aufstockungsstichprobe (Selbständige Welle 5)"
