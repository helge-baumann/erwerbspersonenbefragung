...
Ergebnis <- list()
vars <- names(dat_long)[str_detect(names(dat_long), "_w[:digit:]{0,3}4$")]
vars <- vars[vars != "Faktor_w1234"]
...
dat_long <- dat_long %>%
  mutate(stratum_Alle = "Gesamt",
         ...
         stratum_Bildung = as_factor(S3_w1234))
...
for(b in vars) {
  ...
  if(str_detect(b, "_w[1][:digit:]{0,3}")) {
  name <- which(abgleich[, "n1"] == Ergebnis[[b]]$name)
  }
  if(str_detect(b, "_w[2][:digit:]{0,3}")) {
    name <- which(abgleich[, "n1"] == Ergebnis[[b]]$name)
  }
  if(str_detect(b, "_w[3][:digit:]{0,3}")) {
    name <- which(abgleich[, "n1"] == Ergebnis[[b]]$name)
  }
  if(str_detect(b, "_w[4][:digit:]{0,3}")) {
    name <- which(abgleich[, "n1"] == Ergebnis[[b]]$name)
  }
  Ergebnis[[b]]$namen <- paste0(abgleich[name, paste0("n", 1:4)], " (Welle ", 1:4, ")")
